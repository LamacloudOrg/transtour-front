...
class TravelForm extends Component {
  ...
  componentWillMount() {
    setTimeout(() => console.log("getting order number",100))
    this.props.generateNumber();
    setTimeout(() => console.log("getting order number",200))
    this.props.loadDrivers();
    
  }

  componentDidMount() {
    setTimeout(() => console.log("seteneando estados",this.props.newNumber))
      this.setState({
        error:this.state.error,
        chofer:this.state.chofer,
        patent:this.state.patent,
        isLoading:false,
        orderNumber:this.props.newNumber
      })

      console.log("order Number state",this.state.orderNumber);
  }
 ...
 //   carDriver:Yup.string().required("Requerido"),
    ...
    return (    
      ...
      }
      {
        ...
        initialValues={{ orderNumber: this.state.orderNumber, dateCreated:'', car:this.state.patent, carDriver:this.state.chofer, time:'',company:'', 
  ...
  {props => (
          ...
          <div class="row">
              ...
              <input type="text" defaultValue={props.values.orderNumber}  defaultValue={this.state.orderNumber} className="form-control" name="orderNumber" disabled/>
              </div>
